...
def dnsdumpster(domain):
    try:
        ...
        regex = re.findall(r'httpheaders/\?q=http://(.*?)" data-target="#myModal"', response)
        regexers(domain, regex, 'domain.txt')
        
        regex = re.findall(r'mtr/\?q=(.*?)" data-target="#myModal"', response)
        regexers(domain, regex, 'ip.txt')
    except:pass

dnsdumpster('ui.ac.id')